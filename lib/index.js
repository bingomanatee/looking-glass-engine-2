(function (global, factory) {
	...
	(global = global || self, global.LGE = factory(global.crypto));
	...
	***************************************************************************** */
	...
	class FSM {
	  constructor(name, props, startStateParam) {
	    ...
	    if (Array.isArray(props)) {
	      ...
	      states = lodash.get(props, 'states', []);
	      start = lodash.get(props, 'start', INDETERMINATE);
	      actions = lodash.get(props, 'actions', []);
	    }

	    if (!(states && states instanceof Map)) {
	      if (Array.isArray(states)) {
	        states = lodash(states)
	          .map((item) => {
	  ...
	  addAction(name, fromState, toState) {
	    if (lodash.isFunction(fromState)){
	...
	const noopStream = {
	  next: lodash.identity,
	  error: lodash.identity,
	  complete: lodash.identity
	...
	class Store {
	  ...
	  _waitForEnd() {
	    const endWatch = this.storeState.subscribe(lodash.identity, lodash.identity, () => {
	      this.clearSubs();
	  ...
	  setState(updates, value) {
	    if (is_1.string(updates)) {
	      this.state = {...this.state, [updates]: value};
	    } else {
	      if (!(updates && is_1.object(updates))) {
	  ...
	  addActions(actionsMap = {}) {
	    ...
	    if (actionsMap && typeof actionsMap === 'object') {
	      Object.keys(actionsMap).forEach((name) => {
	        ...
	        if (is_1.function(value)) {
	          this.addAction(name, value);
	        } else if (is_1.object(value)) {
	  ...
	  async update(value, params = [], info) {
	    ...
	    let revertState = clone ? lodash.cloneDeep(this.state) :  {...this.state};
	    ...
	    try {
	      ...
	      } else if (nextState && !is_1.undefined(nextState)) {
	        if (is_1.object(nextState)) {
	  ...
	  _trimTransaction(actionId) {
	    if (!actionId) return;
	    const index = lodash.indexOf(this.transactions, actionId);
	    if (index > -1) {
	...
	class ValueStream {
	    ...
	    if (first && is_1.object(first)) {
	      ...
	      for (let i = 0; i < args.length; ++i) {
	        ...
	        if (i < PARAMETER_NAMES.length) {
	          ...
	          this[key] = value;
	        } else if (value && is_1.object(value)) {
	  ...
	  _emitError(err) {
	    if (!this.isClosed) {
	      if (is_1.string(err)) err = new Error(err);
	  ...
	  _goodType(value) {
	    ...
	    if (is_1.function(this.type)) {
	      return !this.type(value);
	    }
	    if (is_1.string(this.type) && (is_1[this.type])) {
	      return is_1[this.type](value);
	    }
