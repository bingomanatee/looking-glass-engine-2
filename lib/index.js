	...
	***************************************************************************** */
	...
	class Store {
	  ...
	  setState(updates, value) {
	    if (is$1.string(updates)) {
	      this.state = {...this.state, [updates]: value};
	    } else {
	      if (!(updates && is$1.object(updates))) {
	  ...
	  async update(value, params = [], info) {
	    ...
	    try {
	      ...
	      } else if (nextState && !is$1.undefined(nextState)) {
	        if (is$1.object(nextState)) {
